...
Skia also has an experimental SVG back-end, and that's what we're going to use this to build `html2svg`!
...
4. Blink passes the text to [HarfBuzz](https://harfbuzz.github.io) which returns a set of glyph IDs and their relative positions, they're passed to Skia which picks the vectorial data and render it.
